...
/*	 int h  = 5 + rand()%1000;
  ...
  rgba[2] = rand()%255 - 1;
  rgba[3] = 255; 
  /* if { (x1 - width - 3 < 0) draw_line1(image,((x1)/2), y1, x3 , y3, x1 - 2, &rgba,0); }
...
*/
    ...
    draw_line1(image,x1-width/2, y1, x3-width/2 , y3, width, &rgba,0);
    draw_line1(image,x1-width, y1-width/2, x2+width , y2-width/2, width, &rgba,0);
    draw_line1(image,x2+width/2, y1, x4+width/2 , y4, width, &rgba,0);
    draw_line1(image,x4+width/2, y4+width/2, x3-width , y2+width/2, width, &rgba,0);
