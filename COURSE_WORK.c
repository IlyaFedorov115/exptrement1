...
static const char *optstring = "lrpW:E:S:C:A:R:B:ih?";;
...
void draw_line1(struct png* image, int x0, int y0, int x1, int y1, float wd ,png_byte *rgb)
...
/*
void BresenhamCircle(struct png *image, int x0, int y0, int radius, int width, png_byte* rgb) {
    int x = radius; 
    int y = 0;
    int radiusError = 1 - x;
    while (x >= y) {
        set_pixel(image,x + x0, y + y0, rgb); 
        set_pixel(image,y + x0, x + y0, rgb);
        set_pixel(image,-x + x0, y + y0, rgb); 
        set_pixel(image,-y + x0, x + y0, rgb); 
        set_pixel(image,-x + x0, -y + y0, rgb);
        set_pixel(image,-y + x0, -x + y0, rgb); 
...
void draw_star(struct png *image, int x0, int y0, int R, float width, png_byte* rgb){
    ...
    draw_line1(image,array[0][1], array[1][1]-1,array[0][5],array[1][5], width, rgb);
    draw_line1(image,array[0][1], array[1][1]-1,array[0][7],array[1][7]-1, width, rgb);
    draw_line1(image,array[0][3], array[1][3],array[0][7],array[1][7]-1, width, rgb);
    draw_line1(image,array[0][3], array[1][3],array[0][9]+1,array[1][9], width, rgb);
    draw_line1(image,array[0][5], array[1][5],array[0][1],array[1][1], width, rgb);
    draw_line1(image,array[0][5], array[1][5],array[0][9]+1,array[1][9], width, rgb);
...
/*
 ...
 draw_circle(&image, argument.start[0] , argument.start[1], argument.end[0], argument.width, argument.color);
draw_star(&image, argument.start[0], argument.start[1], argument.end[0], argument.width, argument.color);
            }
        ...
        case 'l': /// ТОЖЕ лишние ограничения глянуть !!!!!!!!!!!!
             if(argument.start[0] < 0 || argument.end[0] < 0 ||
                ...
                argument.width < 0)
               {
                ...
                {
                 draw_line1(&image, argument.start[0] , argument.start[1], argument.end[0], argument.end[1], argument.width, argument.color);//возмнжно &color
                }
