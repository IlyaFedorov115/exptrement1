...
void print_help()
{
  ...
  //  printf("%s--copy, -c \t\t%s- делает копию заданного участка. Участок задается при помощи -S  и -E.\n", GREEN,NONE);
   // printf("        Пример: --copy -S 100 100 -E 200 200 fileOut.png\n");
    printf("%s--reflect, -r \t\t%s - выполняет отражения участка относительно заданной оси   \t\t\t  Участок задается при помощи -S и -E,\t\t\t ось задается при помощи -O\n", GREEN,NONE);
    ...
    printf("\n%s--line, -d \t\t %s- рисует линию. Задается координатами начала -S \n \t\t\t и конца -E, шириной -W и цветом -C", GREEN,NONE);
...
void info(struct png* image, char* fileName){
//    printf("%s", fileName);
 //   printf("Width = %d\n", image.width);
 //   printf("Heigth = %d\n", image.height);
    int l = image->width;
 ...
 /* printf(	"┌──────────────────────┐" //1
		...
		"│                      │\n" //8
		"└──────────────────────┘\n", a , l , n);  //12
  */ //Забить циклом по длине названия фала типо фор по длину пишем тире, потом также пробелы и в конце стенку
   printf("┌");
    for (int i = 0; i <= strlen(fileName)+strlen(fileName)/2; i++)
        printf("─");
    printf("┐");
    printf("\n│ ");printf("%s", fileName);
     for (int i = 0; i <= strlen(fileName)/2 - 1; i++)
        printf(" ");
    printf("│");
    printf("\n│ "); printf("Width = %d", l);
    for (int i = 0; i <= strlen(fileName)+strlen(fileName)/2 - 12; i++)
        printf(" ");
    printf("│");
    printf("\n│ "); printf("Heigth = %d", n);
    for (int i = 0; i <= strlen(fileName)+strlen(fileName)/2 - 13; i++)
        printf(" ");
    printf("│");
    printf("\n└");
    for (int i = 0; i <= strlen(fileName)+strlen(fileName)/2; i++)
